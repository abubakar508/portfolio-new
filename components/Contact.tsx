...
const Contact = () => {
    ...
    return (
        <div
            ref={sectionRef}
            className="min-h-screen bg-black text-white py-14 sm:py-16 md:py-20 relative overflow-hidden"
        >
            {/* Background Elements */}
            <motion.div
            ...
            >
                ...
                {/* Contact Section - Optimized for small screens */}
                <div className="flex flex-col lg:flex-row gap-8 sm:gap-12">
                    {/* Left: Contact Info - Mobile optimized */}
                    <motion.div
                        variants={itemVariants}
                        className="w-full lg:w-2/5"
                    >
                        <div className="bg-black/40 backdrop-blur-lg border border-[#FFD700]/20 rounded-xl sm:rounded-2xl p-5 sm:p-6 md:p-8 h-full">
                            <h3 className="text-xl sm:text-2xl font-bold text-[#FFD700] mb-4 sm:mb-6">Contact Information</h3>

                            <div className="space-y-6 sm:space-y-8">
                                {/* Email Addresses - Better for small screens */}
                                <div>
                                        ...
                                        {emailAddresses.map((item, index) => (
                                            ...
                                            >
                                                <a
                                                    ...
                                                    className="flex items-center justify-between border border-[#FFD700]/10 hover:border-[#FFD700]/30 p-2 sm:p-3 rounded-lg transition-all duration-300 bg-black/20 hover:bg-[#FFD700]/5"
                                                >
                                                    <span className="text-gray-300 group-hover:text-white transition-colors text-xs sm:text-sm md:text-base truncate mr-2">{item.email}</span>
                                                    <span className="text-xs sm:text-xs flex-shrink-0 bg-[#FFD700]/10 text-[#FFD700] px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full">{item.label}</span>
                                                </a>
                                            </motion.li>
                                        ))}
                                    </ul>
                                ...
                                {/* Social Links - Spaced better for mobile */}
                                <div>
                                    <h4 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 flex items-center">
                                    ...
                                    <div className="flex gap-3 sm:gap-4">
                                        {["GitHub", "LinkedIn", "Twitter"].map((platform, index) => (
                                            <motion.a
                                                ...
                                                className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-[#FFD700]/10 flex items-center justify-center text-white hover:bg-[#FFD700]/30 transition-all duration-300"
                                            >
                                                {platform === "GitHub" ? (
                                ...
                                >
                                    ...
                                    <p className="text-gray-300 text-sm sm:text-base">Working remotely from Kenya and available worldwide for projects.</p>
                                </motion.div>
                            </div>
                        </div>
                    </motion.div>

                    {/* Right: Contact Form - Mobile optimized */}
                    <motion.div
                        variants={itemVariants}
                        className="w-full lg:w-3/5"
                    >
                        <div className="bg-black/40 backdrop-blur-lg border border-[#FFD700]/20 rounded-xl sm:rounded-2xl p-5 sm:p-6 md:p-8">
                            <h3 className="text-xl sm:text-2xl font-bold text-[#FFD700] mb-4 sm:mb-6">Send Me a Message</h3>

                            {formStatus.isSubmitted ? (
                                <motion.div
                                    initial={{ opacity: 0, y: 20 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    className="bg-[#FFD700]/10 border border-[#FFD700]/30 rounded-lg p-4 sm:p-6 text-center"
                                >
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 sm:h-12 sm:w-12 text-[#FFD700] mx-auto mb-3 sm:mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                    </svg>
                                    <p className="text-base sm:text-lg text-white mb-4">{formStatus.message}</p>
                                    <motion.button
                                        onClick={() => setFormStatus(prev => ({ ...prev, isSubmitted: false }))}
                                        className="px-5 sm:px-6 py-2 bg-[#FFD700] text-black rounded-full text-sm sm:text-base font-medium hover:bg-[#FFD700]/90 transition-all"
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                required
                                                className="w-full bg-black/70 border border-[#FFD700]/20 rounded-lg px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base text-white focus:border-[#FFD700] focus:ring-1 focus:ring-[#FFD700] transition-all outline-none"
                                    ...
                                    <div>
                                        <label htmlFor="phone" className="block text-xs sm:text-sm font-medium text-gray-300 mb-1 sm:mb-2">Phone Number (optional)</label>
                                        <input
                                            type="tel"
                                            id="phone"
                                            name="phone"
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            required
                                            rows={4}
                                            className="w-full bg-black/70 border border-[#FFD700]/20 rounded-lg px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base text-white focus:border-[#FFD700] focus:ring-1 focus:ring-[#FFD700] transition-all outline-none resize-none"
                                            placeholder="Tell me about your project, questions, or just say hi!"
                                        ></textarea>
                                    </div>

                                    {formStatus.isError && (
                                        <div className="bg-red-900/30 border border-red-500/50 text-red-200 p-2 sm:p-3 rounded-lg text-xs sm:text-sm">
                                            {formStatus.message}
                                        </div>
                                    )}
                                    ...
                                    <motion.button
                                        ...
                                        className={`w-full bg-gradient-to-r from-[#FFD700] to-[#B8860B] text-black font-bold py-3 sm:py-4 rounded-lg hover:opacity-90 transition-all text-sm sm:text-base ${formStatus.isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}
